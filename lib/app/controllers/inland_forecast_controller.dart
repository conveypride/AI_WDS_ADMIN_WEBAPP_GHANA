...
import 'package:weather_admin_dashboard/app/model/forecast_dataInland.dart';
import 'package:weather_admin_dashboard/app/routes/app_routes.dart';
import 'package:weather_admin_dashboard/app/services/inland_dailyforecast_ibf_service.dart';
...
final FirebaseFirestore _firestore = FirebaseFirestore.instance;
 ...
 var isTableComplete = false.obs;
  ...
  // --- NEW 12h/24h GENERAL CONDITIONS STATE ---
  var generalConditions = <String, Map<String, dynamic>>{
    "SURFACE WIND": {"12h": "", "24h": ""},
    "VISIBILITY": {"12h": "", "24h": ""},
    "TEMPERATURE": {"12h": "", "24h": ""},
  }.obs;

  void updateGeneralCondition(String param, String period, String value) {
    generalConditions[param]![period] = value;
    generalConditions.refresh();
  }
...
int get totalCellCount => cityData.length * 7; // Increased from 6 to 7
...
void onReady() {
  super.onReady();
  print("Inland: onReady triggered.");
  
  // 1. Check if user is already here
  if (_authCtrl.currentUser.value != null) {
    print("Inland: User already exists. Fetching settings immediately.");
    fetchSettings();
  ...
  ever(_authCtrl.currentUser, (user) {
    if (user != null) {
      print("Inland: Auth state changed! User arrived. Fetching settings.");
      fetchSettings();
 ...
 // ========================================================================
  ...
  Future<void> fetchForecastsAndAnalytics() async {
    try {
      ...
      if (isSuperAdmin) {
        DocumentSnapshot globalStats = await _firestore.collection('analytics').doc('inland_daily_global').get();
        ...
        if (userStats.exists && userStats.data() != null) {
          ...
          if (data['inland_daily_stats'] != null && data['inland_daily_stats'] is Map) {
            final inlandStats = data['inland_daily_stats'] as Map<String, dynamic>;
            kpiTotal.value = inlandStats['total'] ?? 0;
            kpiDraft.value = inlandStats['draft'] ?? 0;
            kpiPending.value = inlandStats['pending_approval'] ?? 0;
            kpiPublished.value = inlandStats['published'] ?? 0;
          }
        }
      }

      // 2. Fetch the FIRST page of forecasts
      Query query = _firestore.collection('inland_daily_forecast')
 ...
 // ========================================================================
  ...
  Future<void> _updateCountersWithBatch(String docId, String newStatus, Map<String, dynamic> payload, bool isSuperAdmin, String authorUid) async {
    final docRef = _firestore.collection('inland_daily_forecast').doc(docId);
    final userRef = _firestore.collection('users').doc(authorUid);
    final globalRef = _firestore.collection('analytics').doc('inland_daily_global');
    ...
    if (oldStatus != newStatus) {
      ...
      batch.set(userRef, { 'inland_daily_stats': statIncrements }, SetOptions(merge: true));
...
// 3. Metadata / General Conditions (Replaced Sector Temperatures)
      if (metadata['generalConditions'] != null) {
        Map<String, dynamic> savedConditions = metadata['generalConditions'];
        
        // Safely map it back to the observable so the UI updates
        generalConditions.value = {
          "SURFACE WIND": Map<String, dynamic>.from(savedConditions['SURFACE WIND'] ?? {"12h": "", "24h": ""}),
          "VISIBILITY": Map<String, dynamic>.from(savedConditions['VISIBILITY'] ?? {"12h": "", "24h": ""}),
          "TEMPERATURE": Map<String, dynamic>.from(savedConditions['TEMPERATURE'] ?? {"12h": "", "24h": ""}),
        };
      } else {
        // Fallback for older forecasts that don't have this data yet
        generalConditions.value = {
          "SURFACE WIND": {"12h": "", "24h": ""},
          "VISIBILITY": {"12h": "", "24h": ""},
          "TEMPERATURE": {"12h": "", "24h": ""},
      ...
      currentForecast.value = InlandForecastData(
...
Future<void> downloadForecastIbf(String docId) async {
  ...
  try {
    ...
    Uint8List? bytes1 = await InlandImageGenerator.generateMapImageFromData(
      mData[p1]?['regions'],
    ...
    Uint8List? bytes2 = await InlandImageGenerator.generateMapImageFromData(
      mData[p2]?['regions'],
    ...
    Uint8List? bytes3 = await InlandImageGenerator.generateMapImageFromData(
      mData[p3]?['regions'],
...
final Map<String, dynamic> ibfPayload = {
  ...
  'metadata': {
    ...
    // ✅ BUG FIXED: Changed from InlandTablePdfService to InlandDailyForecastIbfPdfService
    // ✅ BUG FIXED: Passed ibfPayload instead of forecast
    final Uint8List pdfBytes =
        await InlandDailyForecastIbfPdfService.generateIbfPdf(ibfPayload);
...
Future<void> downloadTableForecastPdfImage(String docId) async {
  ...
  try {
    ...
    final Uint8List pdfBytes = await InlandTablePdfService.generateForecastPdf(forecast);
  ...
  Map<String, dynamic> _buildForecastPayload(String status) {
    ...
    return {
      ...
      'metadata': {
        ...
        // Safely pass the entire 12h/24h map to Firestore
        'generalConditions': Map<String, dynamic>.from(generalConditions),
...
// --> NEW: Sea State Validation
    ...
    // Check if any General Conditions (12h/24h) are missing
    bool hasMissingGeneralConditions = false;
    generalConditions.forEach((param, periods) {
      // Check if the 12h or 24h string is empty or null
      if ((periods['12h']?.toString().trim().isEmpty ?? true) || 
          (periods['24h']?.toString().trim().isEmpty ?? true)) {
        hasMissingGeneralConditions = true;
      }
    });

    if (hasMissingGeneralConditions) {
      Get.snackbar(
        'Missing General Conditions',
        'Please complete all 12h and 24h fields for Surface Wind, Visibility, and Temperature.',
        backgroundColor: Colors.redAccent.withOpacity(0.9),
        colorText: Colors.white,
        snackPosition: SnackPosition.TOP,
        margin: const EdgeInsets.all(16),
    ...
    try {
      ...
      if (editingDocId.value.isNotEmpty) {
        ...
        docId = 'INLAND_${currentForecast.value?.date}_${selectedIssueTime.value}_${user.uid}'.replaceAll('-', '');
      }
      ...
      Get.toNamed(AppRoutes.inlandForecast); 
   ...
   isSubmitting.value = true;
    ...
    try {
      ...
      final docId = 'INLAND_${currentForecast.value?.date}_${selectedIssueTime.value}_${user.uid}'.replaceAll('-', '');
      // Completely safe now!
      ...
      // FIX 3: Redirect to INLAND Unified View (or Dashboard)
      await Future.delayed(const Duration(milliseconds: 500)); 
    Get.toNamed(AppRoutes.inlandForecast); // Redirect to the INLAND Unified View after submission
...
// Reset the new 12h/24h General Conditions
    generalConditions.value = {
      "SURFACE WIND": {"12h": "", "24h": ""},
      "VISIBILITY": {"12h": "", "24h": ""},
      "TEMPERATURE": {"12h": "", "24h": ""},
...
void createNewForecast(){
 ...
 Future<void> downloadCSVTemplate() async {
    List<List<dynamic>> csvData = [
      [
        'CITY', 'SLOT1_WEATHER', 'SLOT1_PROB', 
        'SLOT2_WEATHER', 'SLOT2_PROB',  
        'SLOT3_WEATHER', 'SLOT3_PROB',
        'WIND_DIRECTION' // <-- MOVED TO THE LAST COLUMN
      ]
    ];
    ...
    final anchor = web.HTMLAnchorElement()
      ..href = url
      ..download = "INLAND_Daily_Template.csv";
      ...
      try {
        ...
        for (int i = 1; i < rows.length; i++) {
         ...
         if (cityIndex != -1) {
            ...
            cityData[cityIndex]['slot2_weather']  = row.length > 3 ? row[3].toString().trim() : '';
            cityData[cityIndex]['slot2_prob']     = row.length > 4 ? _formatCSVNumber(row[4]) : ''; 
            cityData[cityIndex]['slot3_weather']  = row.length > 5 ? row[5].toString().trim() : '';
            cityData[cityIndex]['slot3_prob']     = row.length > 6 ? _formatCSVNumber(row[6]) : ''; 
            // <-- SHIFTED WIND IMPORT TO THE LAST INDEX
            cityData[cityIndex]['wind_direction'] = row.length > 7 ? row[7].toString().trim() : ''; 
          }
        }
        cityData.refresh();
        onCellChanged(); 
        tableRefreshTrigger.value = UniqueKey(); 
        Get.snackbar("Import Successful", "Data successfully mapped to table fields.", backgroundColor: const Color(0xFF3DD68C).withOpacity(0.95), colorText: Colors.black);
      } catch (e) {
...
void _autoSetIssueTime() {
 ...
 int get filledCellCount {
    ...
    for (final city in cityData) {
      for (final slot in ['slot1', 'slot2', 'slot3']) { 
        if (city['${slot}_weather']?.toString().trim().isNotEmpty == true) count++;
        if (city['${slot}_prob']?.toString().trim().isNotEmpty == true) count++;
      }
      // Add wind direction to the validation count
      if (city['wind_direction']?.toString().trim().isNotEmpty == true) count++;
    }
    return count;
  }
 ...
 // UPDATED GETTER TO INCLUDE NIGHT
  ...
  // 4. FORECAST METADATA
  // ========================================================================
  var currentForecast = Rxn<InlandForecastData>(InlandForecastData(
    date: DateFormat('yyyy-MM-dd').format(DateTime.now()),
 ...
 Future<void> fetchSettings() async {
    try {
      final user = _authCtrl.currentUser.value;
      if (user == null) {
        print("INLAND Error: fetchSettings aborted because user is null.");
        return; 
      }
      ...
      if (dept.isEmpty || dept == 'all') dept = 'marine'; 
      String docId = '${dept}_settings';

      print("INLAND: Attempting to fetch settings from Firestore doc: $docId");

      DocumentSnapshot doc = await _firestore.collection('settings').doc(docId).get();

      if (doc.exists) {
        print("INLAND: SUCCESS! Settings downloaded.");
        ...
        print("INLAND Warning: Document $docId does NOT exist in Firestore!");
      ...
      print("INLAND CRITICAL ERROR: $e");
      Get.snackbar('Database Error', 'Could not load settings from Firebase.');
    } finally {
      isLoadingSettings.value = false;
      print("INLAND: isLoadingSettings turned off.");
    }
  }
