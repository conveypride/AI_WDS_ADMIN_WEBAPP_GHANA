...
import 'package:weather_admin_dashboard/app/services/mid_week_image_generator.dart';
import 'package:weather_admin_dashboard/app/services/mid_week_ibf_pdf_service.dart';
...
// ========================================================================
  ...
  Future<void> fetchAnalytics() async {
    ...
    Query baseQuery = _firestore.collection('mid_week_forecasts');
  ...
  Future<void> fetchForecastHistory() async {
    try {
      ...
      Query query = _firestore.collection('mid_week_forecasts')
...
// <--- ADD THIS ENTIRE FUNCTION HERE
  ...
  String _calculateValidityString(DateTime start, String issueTime) {
    String valTime = '0600';
    if (issueTime == '1100') valTime = '1200';
  ...
  final finishedRegions = { 'day1': <MidWeekMapRegion>[].obs, 'day2': <MidWeekMapRegion>[].obs, 'day3': <MidWeekMapRegion>[].obs };
  final editablePoints = { 'day1': <MidWeekEditablePoint>[].obs, 'day2': <MidWeekEditablePoint>[].obs, 'day3': <MidWeekEditablePoint>[].obs };
  final mapItems = { 'day1': <MidWeekMapItem>[].obs, 'day2': <MidWeekMapItem>[].obs, 'day3': <MidWeekMapItem>[].obs };
  final _undoStack = { 'day1': <List<MidWeekEditablePoint>>[], 'day2': <List<MidWeekEditablePoint>>[], 'day3': <List<MidWeekEditablePoint>>[] };
  ...
  void addEditablePoint(String p, LatLng point) => editablePoints[p]!.add(MidWeekEditablePoint(point, editablePoints[p]!.length));
  ...
  void finishDrawing(String p) {
    ...
    finishedRegions[p]!.add(MidWeekMapRegion(points: editablePoints[p]!.map((e) => e.position).toList(), color: selectedColor.value));
  ...
  void saveUndoState(String p) => _undoStack[p]!.add(editablePoints[p]!.map((e) => MidWeekEditablePoint(e.position, e.id)).toList());
  ...
  void addMapItem(String p, MidWeekItemType type, String value, LatLng spawnPoint) {
    mapItems[p]!.add(MidWeekMapItem(id: DateTime.now().millisecondsSinceEpoch.toString(), type: type, value: value, position: spawnPoint));
  }
  ...
  MidWeekIBFController() { _initializeTable(); }
  ...
  Future<void> _updateCountersWithBatch(String docId, String newStatus, Map<String, dynamic> payload, String authorUid) async {
    final docRef = _firestore.collection('mid_week_forecasts').doc(docId);
    final userRef = _firestore.collection('users').doc(authorUid);
    final globalRef = _firestore.collection('analytics').doc('mid_week_global');
    ...
    if (oldStatus != newStatus) {
      ...
      batch.set(userRef, { 'mid_week_stats': statIncrements }, SetOptions(merge: true));
  ...
  Future<void> publishForecast({bool isDraft = false}) async {
    ...
    try {
      ...
      if (finalStatus == 'published') {
        await _autoPostMidWeekToCommunityGroups(docId);
      }
      String title = isDraft ? "Draft Saved" : (isSuperAdmin.value ? "Published!" : "Sent for Approval");
      String message = isDraft ? "Forecast safely stored in your drafts." : (isSuperAdmin.value ? "Mid-Week Forecast is now live." : "Forecast routed to supervisor.");
  ...
  Future<void> changeForecastStatus(String id, String status, String authorUid) async {
    try {
      ...
      if (status == 'published') {
        await _autoPostMidWeekToCommunityGroups(id);
      }
        ...
        for (var period in ['day1', 'day2', 'day3']) {
          ...
          if (savedRegions.containsKey(period)) {
            ...
            for (var regionData in periodRegions) {
              ...
              finishedRegions[period]!.add(MidWeekMapRegion(
                points: points,
        ...
        for (var period in ['day1', 'day2', 'day3']) {
          ...
          if (savedMarkers.containsKey(period)) {
            ...
            for (var markerData in periodMarkers) {
              mapItems[period]!.add(MidWeekMapItem(
                id: markerData['id'].toString(),
                type: markerData['type'] == 'icon' ? MidWeekItemType.icon : MidWeekItemType.text,
 ...
 Future<void> downloadForecastPdfAndImage(BuildContext context, Map<String, dynamic> forecast) async {
    try {
      Get.snackbar("Processing", "Generating Mid-Week PDF and Image this takes time (10 mins Approx.), go get some water ...", backgroundColor: Colors.amber.shade700, colorText: Colors.black, duration: const Duration(seconds: 15),);
      
      // 1. Generate the 3-Map Image invisibly
      final Uint8List? mapBytes = await MidWeekImageGenerator.generateThreeMapsImage(
        context: context,
      ...
      final Uint8List pdfBytes = await MidWeekIbfPdfService.generateIbfPdf(forecast, mapBytes);
      ...
      html.AnchorElement(href: pdfUrl)
        ..setAttribute("download", "MidWeek_IBF_$docId.pdf")
        ..click();
      ...
      await for (var page in Printing.raster(pdfBytes, pages: [0], dpi: 300)) {
        ...
        html.AnchorElement(href: imageUrl)
          ..setAttribute("download", "MidWeek_IBF_$docId.png")
          ..click();
  ...
  // ========================================================================
  static const List<String> _midWeekGroupIds = [
    'O9HcbFUOYgAFnxN2OrLN',
    'FXQGcjeQfEJKtreb9cyN',
  ];

  Future<void> _autoPostMidWeekToCommunityGroups(String docId) async {
    const String functionName = 'Mid Week Auto-Post';
    ...
    try {
      ...
      final docSnapshot = await _firestore.collection('mid_week_forecasts').doc(docId).get();
      ...
      try {
        ...
        final mapBytes = await MidWeekImageGenerator.generateThreeMapsImage(
          context: Get.context!,
          regions: mapRegions,
          markers: mapItems,
        ...
        if (mapBytes != null && mapBytes.isNotEmpty) {
          final ibfPdfBytes = await MidWeekIbfPdfService.generateIbfPdf(forecast, mapBytes);
          if (ibfPdfBytes.isNotEmpty) {
            ibfPdfUrl = await _uploadToStorage(
              fileBytes: ibfPdfBytes,
              fileName: 'MidWeek_IBF_${DateTime.now().millisecondsSinceEpoch}.pdf',
              type: 'file',
            );
        ...
        debugPrint('Error generating Mid Week IBF PDF: $e');
      }
      ...
      try {
        ...
        if (mapBytes != null && mapBytes.isNotEmpty) {
          ...
          if (ibfImageBytes.isNotEmpty) {
            ibfImageUrl = await _uploadToStorage(
              ...
              fileName: 'MidWeek_IBF_${DateTime.now().millisecondsSinceEpoch}.png',
              type: 'image',
            );
        ...
        debugPrint('Error generating Mid Week IBF Image: $e');
      }

      // ============ 3. Post URLs to BOTH groups ============
      for (final groupId in _midWeekGroupIds) {
        if (ibfPdfUrl != null) {
          await _postUrlToGroup(
            groupId: groupId,
            mediaUrl: ibfPdfUrl,
            content: 'Mid Week IBF Forecast for $formattedDate',
            type: 'file',
          );
        }
        ...
        if (ibfImageUrl != null) {
          await _postUrlToGroup(
            ...
            content: 'Mid Week IBF Forecast Image for $formattedDate',
            type: 'image',
          );
        }
      }
      ...
      if (postedFiles.isNotEmpty) {
        await _markMidWeekForecastAsPosted(docId);
        Get.snackbar(
          'Auto-Post Complete',
          'Posted: ${postedFiles.join(", ")} to ${_midWeekGroupIds.length} groups',
      ...
      debugPrint('Critical error in Mid Week auto-post: $e');
      Get.snackbar(
        'Auto-Post Error',
  ...
  Future<void> _markMidWeekForecastAsPosted(String docId) async {
    try {
      await _firestore.collection('mid_week_forecasts').doc(docId).update({
        'postedToCommunity': true,
        'postedToCommunityAt': FieldValue.serverTimestamp(),
      });
    } catch (e) {
      debugPrint('Failed to mark Mid Week forecast as posted: $e');
    }
  }
