// lib/app/views/widgets/mid_week_map_widget.dart
...
class MidWeekMapWidget extends StatelessWidget {
  final MidWeekIBFController ctrl;
  final String period;
  final String dateLabel;
  final bool isDark;

  const MidWeekMapWidget({
    super.key, 
    required this.ctrl, 
      ...
      children: [
          ...
          child: Row(
            ...
            children: [
              PopupMenuButton<String>(
                ...
                onSelected: (iconName) => _spawnItem(MidWeekItemType.icon, iconName, mapController),
              ...
              PopupMenuButton<String>(
                ...
                onSelected: (letter) => _spawnItem(MidWeekItemType.text, letter, mapController),
                color: isDark ? Colors.grey.shade800 : Colors.white,
              ...
              child: Container(
                ...
                child: FlutterMap(
                  ...
                  children: [
                    ...
                    PolygonLayer(
                      polygons: regions.map((region) => Polygon(points: region.points, color: _getColorFromString(region.color).withOpacity(0.3), borderColor: _getColorFromString(region.color), borderStrokeWidth: 2.0)).toList(),
                    ),

                    if (activePoints.isNotEmpty)
                      PolygonLayer(
                        polygons: [Polygon(points: activePoints.map((e) => e.position).toList(), color: ctrl.activeColor.withOpacity(0.3), borderColor: ctrl.activeColor, borderStrokeWidth: 2.0)]
                      ),
        ...
        Container(
          ...
          child: Obx(() {
            ...
            if (!isDrawingThis) {
              return Center(child: TextButton.icon(onPressed: () => ctrl.startDrawing(period), icon: Icon(PhosphorIcons.pencilSimple(), size: 16), label: const Text("Draw Risk Area", style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold))));
            }
            ...
            return FittedBox(
              ...
              child: Row(
                ...
                children: [
                  ...
                  Row(
                    children: [
                      IconButton(tooltip: "Undo", icon: const Icon(Icons.undo, size: 16), onPressed: () => ctrl.undo(period), padding: EdgeInsets.zero, constraints: const BoxConstraints()),
                      const SizedBox(width: 8),
                      IconButton(tooltip: "Delete", icon: const Icon(Icons.delete_outline, size: 16, color: Colors.red), onPressed: () => ctrl.deleteActiveDrawing(period), padding: EdgeInsets.zero, constraints: const BoxConstraints()),
                      const SizedBox(width: 8),
                      IconButton(tooltip: "Cancel", icon: const Icon(Icons.close, size: 16, color: Colors.orange), onPressed: () => ctrl.cancelDrawing(period), padding: EdgeInsets.zero, constraints: const BoxConstraints()),
                      const SizedBox(width: 8),
  ...
  void _spawnItem(MidWeekItemType type, String value, MapController mapCtrl) => ctrl.addMapItem(period, type, value, mapCtrl.camera.center);
  ...
  Widget _buildItemVisual(MidWeekMapItem item) {
    if (item.type == MidWeekItemType.text) return Text(item.value, style: TextStyle(fontSize: 24, fontWeight: FontWeight.w900, color: Colors.black, shadows: const [Shadow(color: Colors.white, blurRadius: 4), Shadow(color: Colors.black54, offset: Offset(1, 1), blurRadius: 2)]));
    IconData iconData;
    switch (item.value) {
